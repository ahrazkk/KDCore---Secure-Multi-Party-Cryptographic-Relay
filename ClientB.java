...
public class ClientB {
    private static String myID = "ID_B";
    private static long lastTimestamp = 0;

    public static void main(String[] args) throws Exception {
        // --- PHASE 1 : STEP 0 - SETUP CLIENT B RSA KEYS ---
        ...
        System.out.println("--- CLIENT B INITIALIZED ---");
        System.out.println("Public Key (PU_B): " + Base64.getEncoder().encodeToString(myPU.getEncoded()));
        System.out.println("Private Key (PR_B): " + Base64.getEncoder().encodeToString(myPR.getEncoded()));
        System.out.println("-----------------------------");
        ...
        byte[] encryptedNK2 = (byte[]) in.readObject();
        Cipher rsa = Cipher.getInstance("RSA");
        rsa.init(Cipher.DECRYPT_MODE, myPR);
        String nk2 = new String(rsa.doFinal(encryptedNK2)).split("\\|\\|")[0];

        // --- PHASE 1 : STEP 3 - SEND ENCRYPTED NONCES [NB || NK2] ---
        rsa.init(Cipher.ENCRYPT_MODE, puK); 
        out.writeObject(rsa.doFinal(("NB_CLIENT_B||" + nk2).getBytes()));

        // --- PHASE 1 : STEP 5 - RECEIVE AND DECRYPT MASTER KEY KB ---
        byte[] kbData = (byte[]) in.readObject();
        rsa.init(Cipher.DECRYPT_MODE, myPR);
        String kb = new String(rsa.doFinal(kbData));
        System.out.println("PHASE 1 SUCCESS: Established Master Key KB -> " + kb);

        // --- KEY FIX: Ensure Master Key is exactly 16 bytes for AES ---
        byte[] keyBytes = Arrays.copyOf(kb.getBytes(), 16); 
